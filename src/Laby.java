...
public class Laby
{
   ...
   public void bouger(int k)
   {
      ...
      if(k == 80) // Potion
      {
         pers.getInv().use(new PotionHeal());
      }
...
*/
     ...
     Sublaby s = bestSublaby();

      for(int i = 0 ; i < taille ; i++)
         for(int j = 0 ; j < taille ; j++)
            if(!board[i][j].hasSublaby(s))
   ...
   private void createSublaby(int x, int y)
   {

      Sublaby s = new Sublaby(taille);

      if(sublabys == null)
      {
         this.sublabys = new Sublaby[1];
         sublabys[0] = s;
      }
      else
      {
         Sublaby []newSub = new Sublaby[sublabys.length+1];
         System.arraycopy(sublabys, 0, newSub, 0, sublabys.length);

         newSub[sublabys.length] = s;
         sublabys = newSub;
      }
...
/*
  ...
  */
      ...
      for(int i = 0 ; i < sublabys.length ; i++)
         if(sublabys[i].nbDalle() > max)
         {
            max = sublabys[i].nbDalle();
            s = sublabys[i];
         }
